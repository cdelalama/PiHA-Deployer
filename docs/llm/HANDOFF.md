...
- `home-assistant/mariadb/README.md` now promotes the manual NAS bootstrap flow (with the single-command helper) and the helper script works on BusyBox/bash 3 shells, logs the SSH step, skips SSH when NAS_SSH_HOST is local, and copies files directly when local
...
- home-assistant/mariadb/setup-nas-mariadb.sh: 1.0.5
